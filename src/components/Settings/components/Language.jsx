...
import { Earth, ChevronsUpDown } from "lucide-react";
import {
  Button,
  Dropdown,
  DropdownItem,
    ...
    <ItemWrapper title={t('settings.general.language')}>
      <div className="flex justify-between items-center gap-2 bg-content1 dark:bg-content2/30 p-2">
        <div className="flex items-center gap-2">
          ...
          <div className="text-sm text-foreground">{t('settings.general.interfaceLanguage')}</div>
        </div>
        <Dropdown>
          <DropdownTrigger>
            <Button className="capitalize" variant="flat" size="sm" endContent={<ChevronsUpDown className="size-4" />}>
